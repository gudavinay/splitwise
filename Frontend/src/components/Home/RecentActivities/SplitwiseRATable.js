...
import { getUserID, getMonthFromUtils, getDateFromUtils, getUserCurrency } from '../../Services/ControllerUtils'
              ...
              <Row>
                ...
                <Col>
                  ...
                  {row.paid_by.settled !== 'N' ? (<div>
                    ...
                    <strong>{row.paid_by._id === getUserID() ? "You" : row.paid_by.name}</strong> settled up <strong>{userPreferredCurrency} {row.amount}</strong> for <strong>"{row.description}"</strong> in <strong>"{row.group_id.name}"</strong>.<br />
                    <span style={{ color: '#999', fontSize: '12px' }}>{getMonthFromUtils(row.updated_date)} {getDateFromUtils(row.updated_date)} </span>
                  </div>)}
                </Col>
              </Row>
            </TableCell>
